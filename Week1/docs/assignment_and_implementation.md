...
同级 `C:/Users/goahe/Desktop/ID2221/datasets` 按用户要求保留为镜像。项目实现只读取 `Week1/datasets/`，不依赖或自动同步镜像；两边目前 8 个文件的 SHA-256 逐一相同。
...
- 不做无依据的插值。天气或空气质量没有匹配时保留行程，环境字段为 `NULL`，并写入 `weather_match_status`、`air_quality_match_status` 和时间差字段。
...
通用部分只需要负责：文件发现与格式读取、显式 schema 校验、通用列名规范化、运行元数据、质量结果汇总、Delta 写入和错误处理。每个数据集的差异通过一份简短配置和一个必要时才存在的转换函数表达：
...
`ingestion_runs` 至少记录 `run_id`、`dataset_name`、`source_file`、`source_sha256`、`schema_version`、读入/写出/拒绝行数、开始/结束时间、状态和错误摘要。相同源哈希与 schema 版本已经成功处理时，应跳过或明确覆盖，保证幂等性。

第一版代码控制在下面五个职责文件内；只有出现第二个真实调用方或独立变化原因时再拆分：

```text
src/urban_data/
├── main.py         CLI、SparkSession 和运行编排
├── ingest.py       通用读取、schema 校验、元数据和 Delta 写入
...
- **直接拒绝或隔离：** schema 不匹配、主键/核心时间缺失、无法解析的时间、下车时间不晚于上车时间、区域 ID 无法匹配。
...
两种版本必须使用相同 schema、相同数据、相同 Spark 配置和相同文件压缩格式。每个查询至少运行 3 次，区分首次冷启动和后续运行，并报告中位数。记录：

- 从读取源文件到 Delta 提交完成的墙钟时间；
- Delta 目录总字节数；
- 实际数据文件数量和平均文件大小；
- 三条指定查询的延迟与物理执行计划。

指定查询为：每个上车行政区的行程数；每天的平均行程时长；每个上车行政区的平均车费。三条都是全范围聚合，因此分区未必会加速；这个结果本身就是需要讨论的工程结论。
...
| 路径 | 角色 | 项目是否读取 | 维护规则 |
| --- | --- | --- | --- |
| `C:/Users/goahe/Desktop/ID2221/Week1/datasets` | 项目运行数据副本 | 是 | 所有代码使用项目根相对路径 `datasets/...` |
| `C:/Users/goahe/Desktop/ID2221/datasets` | 用户要求保留的镜像 | 否 | 不删除、不改写、不自动同步，不作为运行依赖 |
...
| 记录级 quarantine | 核心时间无法解析、下车不晚于上车、必填键缺失、区域键不在维表 | 保留原记录、拒绝原因、来源文件和运行 ID |
| 保留并告警 | 负车费、负总金额、文件月份外时间、极端距离 | 第一版不自动删除；统计并等待业务规则确认 |
| 允许为空 | 已确认可选的出租车字段、缺失天气或空气质量匹配 | 保留 `NULL`，增加匹配状态，不替换成 0 |
| 阻断集成但不阻断摄取 | 天气时区未确认 | 可生成天气 Silver 表，但不发布带天气字段的最终 Gold 表 |
...
1. 用相同 Silver 输入生成未分区和按 `pickup_month` 分区两版出租车 Delta 表。
2. 固定 Spark 配置、压缩格式和目标文件大小；分别记录摄取时间、目录字节数和数据文件数。
3. 三条指定查询分别运行一次冷启动和至少三次热运行，报告中位数及执行计划。
4. 只有实测证明月分区对目标查询或维护有益时，才将其确定为生产设计。

**通过门：** 原始指标、查询 SQL、执行计划和结论能互相追溯；不把“分区更多”当作性能更好。

#### 阶段 G：完成交付物

1. 从本实现文档压缩出 3–5 页设计报告，不复制整份工作记录。
