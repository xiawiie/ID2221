...
- Spark + Delta 摄取、Silver 标准化、Gold 集成、验证、配置测试和存储基准已在 **Windows 原生**环境完成；数据画像脚本读取项目内 `datasets/`。
- 运行环境：Conda `id2221-week1`、OpenJDK 17、Python 3.12、PySpark 3.5.7、delta-spark 3.3.2、`scripts/run_win.ps1`；Windows Hadoop 辅助在 `tools/hadoop/`。
- 阶段 8 摄取结果（2026-09-12 Windows）：三个月出租车分别接受 2,963,754 / 3,006,723 / 3,581,500 行、隔离 870 / 803 / 1,128 行；区域 265/0；天气 8,784/0；空气质量纽约子集 51,885 行聚合成 8,784 个 UTC 小时。统一验收通过，证据在 `artifacts/ingestion_verification.json`。
...
- 基准 Windows 本机结果（2026-09-12，`artifacts/benchmark_report.json`）：未分区 821.5 MiB / 20 文件 / 写入 14.4 s；分区 827.4 MiB / 46 文件 / 写入 10.8 s。三条全范围查询中位数几乎持平（未分区 354–402 ms，分区 340–400 ms），结果 SHA-256 一致。默认推荐仍为未分区（文件更少、运维更简单）。WSL 早期结果（分区查询约 2× 慢）不再作为当前结论。
- 出租车源没有可证明唯一的自然键；标准化 `trip_id` 指纹存在 1 个重复值，因此只作为审计线索，不作为去重或主键承诺。
- 重复执行均按相同源哈希和 schema 版本跳过，幂等检查生效；成功元数据已写入 `lakehouse/metadata/ingestion_runs/`。
- 测试使用标准库 `unittest`；Windows 命令：`PYTHONPATH=src python -m unittest discover -s tests`（经 `run_win.ps1` 或 Conda python）。
- `Week1/datasets` 是项目运行副本；可选镜像目录 `D:/DD2221/data/` 不作为运行依赖。
- `.gitignore` 忽略大型数据、`lakehouse/` 和 Conda 缓存；`artifacts/*.json` 证据可跟踪。
...
## 宿主机与运行环境
- CPU：AMD Ryzen 9 8945HX；内存约 34 GiB。
- 宿主机 Python 3.13（Anaconda）用于 `profile_data.py` 画像。
- Spark 流水线主环境：**Windows 11 原生**、Conda `id2221-week1`、OpenJDK 17、PySpark 3.5.7、delta-spark 3.3.2、`HADOOP_HOME=tools/hadoop`。
- 本地 Spark 配置为 `local[8]`、Driver 4 GB；ingest、integrate、benchmark 均已通过。
- WSL / `.venv-wsl` 为可选 legacy 路径，见 README Legacy 节。
...
- 指定三条全范围查询在 Windows 本机几乎持平；默认仍推荐未分区（文件更少）。月分区只有在过滤或保留成为主要负载时再重新评估。

## 阶段 12 代码修复（2026-09-12 续）
- **物理 schema 校验：** 新增 `validation.py`；Parquet/CSV 在 typed read 前先比对物理列名。
- **ingestion 元数据：** `skipped` / `failed` 运行写入 `ingestion_runs`。
- **quarantine 一致性：** 同一 `_source_file` 重跑且无隔离行时，Delta delete 清除旧 quarantine。
- **空气质量候选键：** 重复复合键进入 quarantine。
- **Gold 审计字段：** 保留 `weather_observation_ts_local`、`air_observation_ts_utc`，新增 lag 分钟字段。
- **benchmark 存储统计：** 改用 Delta `DESCRIBE DETAIL`，避免 obsolete 文件计数。
- **测试：** 新增 `tests/test_validation.py`（9 tests OK）。
...
| 运行态 | changed-and-verified | Windows Conda + JDK 17 + PySpark 3.5.7 + Delta 3.3.2 已完成摄取、集成和基准 |
| 文档 | changed-and-verified | README、design_report（扩充版）、benchmark_report 与 Windows 结果一致 |
